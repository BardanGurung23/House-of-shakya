...
const projects = [
  {
    ...
    description: "Lakeside luxury living with panoramic Phewa Lake and Annapurna range views.",
  ...
  },
  {
    ...
    description: "Modern mid-rise apartments in the heart of Sedi with world-class amenities.",
  ...
  },
  {
    ...
    description: "Lakefront plots with full road infrastructure in Begnas's pristine landscape.",
  ...
  },
  {
    ...
    status: "Ongoing",
    statusColor: "bg-gold/80 text-navy-deep",
    description: "Exclusive hillside homes with sunrise views over the Himalayas and Phewa Valley.",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {projects.map((project, i) => (
            <Reveal key={project.id} delay={i * 0.08} className={project.span}>
              <div className="group relative rounded-xl overflow-hidden img-zoom cursor-pointer"
                ...
                <div className="absolute inset-0 transition-opacity duration-300"
                  style={{ background: "linear-gradient(to top, oklch(0.18 0.05 255 / 0.9) 0%, oklch(0.18 0.05 255 / 0.2) 60%, transparent 100%)" }} />

                <div className="absolute inset-0 p-6 flex flex-col justify-between">
                  {/* Status badge */}
                  <div>
                    <span className={`inline-block px-2.5 py-1 text-xs font-semibold rounded-full ${project.statusColor}`}>
                      {project.status}
                    </span>
                  </div>
                  ...
                  {/* Info */}
                  <div>
                    <p className="text-xs text-cream/60 mb-1 uppercase tracking-widest">{project.type}</p>
                    <h3 className="text-lg md:text-xl font-semibold text-cream mb-1" style={{ fontFamily: "'Playfair Display', serif" }}>
                      {project.name}
                    </h3>
