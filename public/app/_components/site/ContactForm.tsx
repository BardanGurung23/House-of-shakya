      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div>
              ...
              {[
                {
                  icon: MapPin,
                  title: "Address",
                  lines: ["New Road, Pokhara — 33700", "Gandaki Province, Nepal"],
                },
                {
                  icon: Mail,
                  title: "Email",
                  lines: ["sales@yourshousing.com"],
                  link: "mailto:sales@yourshousing.com",
                },
                {
                  icon: Phone,
                  title: "Phone",
                  lines: ["+977 98690 28924"],
                  link: "tel:+9779869028924",
                },
                {
                  icon: Clock,
                  title: "Office Hours",
                  lines: ["Sun – Fri: 10:00 AM – 6:00 PM", "Saturday: By appointment"],
                },
              ].map((item, i) => (
                <Reveal key={item.title} delay={i * 0.08}>
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-widest text-navy/40 mb-1">{item.title}</p>
                      {item.lines.map((line, j) => (
                        item.link && j === 0 ? (
                          <a key={j} href={item.link} className="block text-sm text-navy-deep hover:text-forest transition-colors">
                            {line}
                          </a>
                        ) : (
                          <p key={j} className="text-sm text-navy/70">{line}</p>
                        )
                      ))}
                    </div>
                  </div>
            ...
            {/* Map placeholder */}
            <Reveal delay={0.3}>
              <div className="mt-10 h-52 rounded-xl overflow-hidden border"
          ...
          {/* Form */}
          <Reveal delay={0.1}>
            <div className="rounded-2xl p-8 md:p-10 border shadow-card bg-cream"
              style={{ borderColor: "oklch(0.26 0.07 258 / 0.1)" }}>
              {submitted ? (
                <div className="flex flex-col items-center justify-center h-full py-16 text-center">
                  <CheckCircle2 size={52} className="text-forest mb-5" />
                  <h3 className="text-2xl font-semibold text-navy-deep mb-3" style={{ fontFamily: "'Playfair Display', serif" }}>
                    Message Sent
                  </h3>
                ...
                <>
                  <h3 className="text-xl font-semibold text-navy-deep mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>
                    Send a Message
                  </h3>
                  <p className="text-sm text-navy/50 mb-8">We typically respond within 4 business hours.</p>

                  <form onSubmit={handleSubmit} className="space-y-6">
                    {[
                      { id: "name", label: "Full Name", type: "text", placeholder: "Your full name" },
                      { id: "email", label: "Email Address", type: "email", placeholder: "you@example.com" },
                      { id: "phone", label: "Phone Number", type: "tel", placeholder: "+977 98XXXXXXXX" },
                    ].map((field) => (
                      <div key={field.id}>
                        <label htmlFor={field.id} className="block text-xs font-semibold uppercase tracking-widest text-navy/50 mb-2">
                          {field.label}
                        </label>
                        <input
                          id={field.id}
                    ...
                    <div>
                      <label htmlFor="message" className="block text-xs font-semibold uppercase tracking-widest text-navy/50 mb-2">
                        Message
                      </label>
                      <textarea
                        id="message"
