...
import { useListAllInterviewQuery } from "@/redux/services/interview";
...
export default function AddEditSeoForm({
  ...
  const { data: allInterview, isSuccess: interviewSuccess } =
  ...
  // useEffect(() => {
  //   if (interviewSuccess) {
  //     const data = allInterview?.data?.data?.map((each) => {
  //       return {
  //         label: `${each.initials} > ${each.slug}`,
  //         value: each.slug,
  //       };
  //     });
  //     setPageOptions((prev) => {
  ...
  //         (option) =>
  //           option.label === "Interview Detail Page" &&
  //           option.value === "interview-detail-page"
  //       );
  ...
  //           ...prev,
  //           {
  //             label: "Interview Detail Page",
  //             value: "interview-detail-page",
  //             options: data,
  //           },
  //         ];
  //       }

  //       // Return the same array if the entry already exists
  //       return prev;
  //     });
  //   }
  // }, [interviewSuccess]);
      ...
      <div className="flex gap-[1.5rem] mb-[2.5rem]">
        ...
        >
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <Select {...field} options={PageOptions} label="Page" />
            )}
          />
