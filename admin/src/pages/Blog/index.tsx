...
import { BLOG_URL } from "@/constants/apiUrlConstants";
import usePagination from "@/hooks/usePagination";
import { useDeleteApiMutation, useGetApiQuery } from "@/redux/services/crudApi";
import { BLOG_ADD_ROUTE } from "@/routes/routeNames";
...
export default function Blog() {
  ...
  const accessList = checkAccess("Blog");
  ...
  const {
    ...
    isSuccess: success,
    refetch,
  } = useGetApiQuery(`${BLOG_URL}list`);
  const [deleteBanner] = useDeleteApiMutation();

  const handleNewUser = (id: number | null) => {
    id === null ? navigate(BLOG_ADD_ROUTE) : navigate(`${BLOG_ADD_ROUTE}${id}`);
  };
  ...
  const handleDelete = async () => {
    try {
      const response = await deleteBanner(`${BLOG_URL}${deleteId}`).unwrap();
      handleResponse({
        res: response,
        onSuccess: () => {},
      });
  ...
  const tableData =
    success && allBlogs?.data?.data
      ? allBlogs?.data?.data.map(({ id, title, author, createdAt }) => [
          title,
          author,
