...
import { CAREER_CATEGORY_ADD_ROUTE } from "@/routes/routeNames";
...
export default function CareerCategory() {
  ...
  const {
    ...
    isSuccess: success,
    refetch,
  } = useGetApiQuery(`${CAREER_CATEGORY_URL}list`);
  const [deleteBanner] = useDeleteApiMutation();

  const handleNewUser = (id: number | null) => {
    id === null
      ? navigate(CAREER_CATEGORY_ADD_ROUTE)
      : navigate(`${CAREER_CATEGORY_ADD_ROUTE}${id}`);
  };
  ...
  return (
    <>
      <PageTitle title="Career Category" />
      <PageHeader
        hasAddButton={accessList.includes("add")}
        newButtonText="Add New Career Category"
        handleNewButton={() => handleNewUser(null)}
        handleReloadButton={handleReload}
        hasSubText
        subText="Add Comprehensive Career Category in Each Section"
      />
