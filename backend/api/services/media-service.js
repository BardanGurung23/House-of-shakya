...
const uploadMedia = async (req) => {
  try {
    ...
    if (req?.user?.id) req.createdBy = req.user.id;
  ...
  try {
    ...
    if (usedPaths.length > 0) {
      return {
        status: httpStatus.NOT_ACCEPTABLE,
        success: false,
        message: "Some media paths are in use in other modules.",
        data: usedPaths,
      };
    }
    ...
    throw err; // Pass error to the error-handling middleware
  }
};
